...
class MultiCarousel extends LitElement {
  ...
  connectedCallback() {
    ...
    this.cssliderStyles = html`
      ${this.opArr
        .map(
          val =>
            `.csslider > input:nth-of-type(${val}):checked ~ ul li:first-of-type { margin-left: -${(val -
              1) *
              100}%; }`
        )
        .join("\n")}
    `;
    this.cssliderInputStyles = html`
      ${this.opArr
        .map(
          val =>
      ...
      { opacity: 1; }
    `;
    this.cssliderInputCheckedStyles = html`
      .csslider.infinity > input:first-of-type:checked ~ .arrows
      label.goto-last,
      ${this.opArr
        .map(
          val =>
            `.csslider > input:nth-of-type(${val}):checked ~ .arrows > label:nth-of-type(${val -
              1})`
        )
        .join(", ")}
    ...
    this.cssliderInfinityStyles = html`
      .csslider.infinity > input:last-of-type:checked ~ .arrows
      label.goto-first,
      ${this.opArr
        .map(
          val =>
    ...
    this.htmlInputs = `${this.opArr.map(
      val =>
        html`
          <input type="radio" name="slides" id="slides_${val}" />
        `
    )}`;
  }
    ...
    return css`
      ...
      @font-face {
        ...
        src: local("Raleway"), local("Raleway-Regular"),
          url(https://fonts.gstatic.com/s/raleway/v13/1Ptug8zYS_SKggPNyC0ISg.ttf)
            format("truetype");
      }
      @font-face {
        font-family: "Raleway";
        ...
        src: local("Raleway Bold"), local("Raleway-Bold"),
          url(https://fonts.gstatic.com/s/raleway/v13/1Ptrg8zYS_SKggPNwJYtWqZPBQ.ttf)
            format("truetype");
      }
      ::-webkit-scrollbar {
        width: 2px;
  ...
  nextslide(ev) {
    ...
    if (masterId === this.masterId) {
      ...
      this.shadowRoot
        .querySelector('label[for="slides_' + slide + '"]')
        .click();
    }
  }
  ...
  _getNav() {
    ...
    let arrows = html`
      <div class="arrows">
        ...
        <label
          class="goto-first ${arrowClass}"
          for="slides_1"
          @click="${this._gotonav}"
        ></label>
        <label
          class="goto-last ${arrowClass}"
          for="slides_${this.numslides - 1}"
          @click="${this._gotonav}"
        ></label>
      </div>
    `;
      ...
      <div class="navigation">
        <div>
          ${this.opArr.map(
            val =>
              html`
                <label for="slides_${val}" @click="${this._gotonav}"></label>
              `
          )}
        </div>
      </div>
      ...
      <div class="csslider infinity" id="theslider">
        ${this.opArr.map(val => {
          ...
          if (val !== 1) {
            ...
            res = html`
              <input type="radio" name="slides" id="slides_${val}" checked />
            `;
          }
          return res;
        })}
